      ...
      function($scope, $state, $stateParams, moreService, $compile, $timeout) {
        ...
        $scope.captrueCarList = function(value) {
          ...
          promise.then(function(resp) {
            if (resp.resultCode == 200 && resp.data.nonAcrossVehicle) {
              $scope.nonmotorListData = resp.data.nonAcrossVehicle.list;
              $scope.page1.totalRow = resp.data.nonAcrossVehicle.totalRow
            } else {
              $scope.nonmotorListData = [];
              $scope.page1.totalRow = 0;
            }
          }).catch(function() {}).finally(function() {
            // layer.close(index);
            $scope.loader = false;
